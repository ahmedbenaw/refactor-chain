...
Every governance lane (Java, Web, UI, code) has been independently reauthored from industry-standard practice. The JVM lane now applies to **any** Maven/Gradle project and the Web lane to any React/Vue/Svelte/Angular project — no org-specific rules anywhere. Repository history was rebuilt clean for this release.
