...
## Harness-enforced discipline (not just documented contracts)

Three discipline-pack behaviors are enforced by the harness itself, so they cannot be skipped:
- **plan-gate** — `orchestrate.mjs baseline` refuses to run until `state.planNote` is set (via `init --plan-note` or `baseline --plan-note`). No safety net, no lane, without a written mini-plan.
- **adversarial-verify** — `orchestrate.mjs advance` refuses `--delta legal` without the `--adversarial` flag: a step's "it worked" claim cannot be recorded until it survived the attack pass.
- **scope-fence** — the `guard.mjs` PostToolUse hook hard-flags any out-of-scope edit into `state.notes` (via `orchestrate.mjs flag-drift`), so drift is durably visible to the write-up and improvement retro.

The remaining discipline skills (live-truth, ruthless-editor, memory, guidelines-contract) are standing contracts the agent follows; memory and scope-fence additionally have dedicated hooks (SessionEnd capture / SessionStart recall; PostToolUse drift flag).
