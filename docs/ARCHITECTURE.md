...
- **java** — nine ordered governance steps for JVM back-ends (architecture,
  module-rename, dao-model, service, controller, dependency-guard, api-naming,
  common-extract, code-optimize). Gradle projects route here too, unless they
  are Android apps — those stay in the mobile UI lane.
...
A composite action any repo can call (`uses: ahmedbenaw/refactor-chain@v3`).
...
`refactor-plan-gate` pauses on the signal for the Integrate-vs-Adopt decision
checkpoint (heuristic + both modes specified in
