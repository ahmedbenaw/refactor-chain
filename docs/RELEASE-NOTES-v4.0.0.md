...
Every governance lane (Java, Web, UI, code) has been independently reauthored from public, industry-standard best practice. The backend lane now applies to **any** registry-backend stack and the Web lane to any React/Vue/Svelte/Angular project — no org-specific rules anywhere. Repository history was rebuilt clean for this release.
