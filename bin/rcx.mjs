...
// ---------------------------------------------------- hook registration
// Register/refresh the 5 manifest hooks in a settings/hooks JSON file.
...
function registerHooks(jsonPath, scriptsDir, hooks) {
  ...
  for (const h of hooks) {
    ...
    s.hooks[h.event] = (s.hooks[h.event] || []).filter(
      (b) => !(b.hooks || []).some((x) => (x.command || "").includes("refactor-chain/scripts/"))
    );
    s.hooks[h.event].push({
      matcher: h.matcher,
      hooks: [{ type: "command", command: `${nodeCmd} ${scriptsDir}/${file}`, timeout: 10 }],
    });
  }
...
function installClaude(root, hooks) {
  ...
  // scripts (+ lib) into the orchestrator skill dir (space-free path)
