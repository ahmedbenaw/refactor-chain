...
function printHelp() {
  ...
  say("  --update         check the installed version, then reinstall cleanly");
  say("  --uninstall      remove refactor-chain from each surface");
}

// Deferred help: the arg-parse loop runs during module evaluation, BEFORE the say/c consts
// above are initialized — calling printHelp() there threw a TDZ ReferenceError. Call it here.
...
// ---------------------------------------------------------------- 1. source
let SRC = "";
let RCX_VERSION = ""; // source version, read once SRC is resolved (for the stamp + --update)
let TMP = "";
...
// ---------------------------------------------------- copy helpers
// Never copy a maintainer's local secrets or VCS/deps into an install target: when run from a
// working clone, the source contains a token-bearing .env (gitignored) and .git.
const COPY_EXCLUDE = new Set([".git", "node_modules"]);
const isExcluded = (name) => COPY_EXCLUDE.has(name) || name === ".env" || name.startsWith(".env.");
...
function installClaude(root, hooks) {
  ...
  // verify the harness scripts actually landed BEFORE stamping / registering hooks — a
  // partial copy must fail loudly, not leave 6 dangling hooks pointing at missing files.
  if (!exists(path.join(scriptsDir, "orchestrate.mjs")) || !exists(path.join(scriptsDir, "lib", "board.mjs"))) {
    bad(`Claude install incomplete — harness scripts did not copy (source: ${SRC}). Skipping hook registration to avoid dangling hooks.`);
    return;
  }
  // version stamp — read by --update to report the installed version
  try { if (RCX_VERSION) fs.writeFileSync(path.join(D, "skills", "refactor-chain", ".rcx-version"), RCX_VERSION + "\n"); } catch { /* non-fatal */ }
  // commands/*
...
function installCodex(codexHome, hooks) {
  ...
  copyDir(SRC, pluginDir); // copyDir excludes .env*/.git/node_modules (no secret/VCS leak)
...
async function main() {
  ...
  try { RCX_VERSION = JSON.parse(fs.readFileSync(path.join(SRC, ".claude-plugin", "plugin.json"), "utf8")).version || ""; } catch { RCX_VERSION = ""; }
  ...
  head(opts.uninstall ? "Removing…" : opts.update ? "Updating…" : "Installing…");
  if (opts.uninstall) {
  ...
  if (opts.update) {
    ...
    if (planClaude) {
      let cur = "";
      try { cur = fs.readFileSync(path.join(CLAUDE_HOME, "skills", "refactor-chain", ".rcx-version"), "utf8").trim(); } catch { /* none */ }
      if (cur) ok(`current: v${cur}  →  v${RCX_VERSION || "?"}`);
      else skip(`no version stamp (fresh or pre-4.6.3 install) — installing v${RCX_VERSION || "?"}`);
      try { for (const e of fs.readdirSync(path.join(CLAUDE_HOME, "skills"))) if (e.startsWith("refactor-")) fs.rmSync(path.join(CLAUDE_HOME, "skills", e), { recursive: true, force: true }); } catch { /* none */ }
      try { for (const e of fs.readdirSync(path.join(CLAUDE_HOME, "commands"))) if (e.startsWith("refactor") || e === "fix.md" || e === "check.md") fs.rmSync(path.join(CLAUDE_HOME, "commands", e), { force: true }); } catch { /* none */ }
    }
    if (planCodex) { try { fs.rmSync(path.join(CODEX_HOME, "plugins", "refactor-chain"), { recursive: true, force: true }); } catch { /* none */ } }
  }
  ...
  // 8. verify + summary
  head(opts.update ? `Updated to v${RCX_VERSION || "?"}.` : "Done.");
