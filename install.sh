...
#
# Flags: --yes --dry-run --update --only <ids> --skip <ids> --owner <name> --details --uninstall
set -eu

OWNER="ahmedbenaw"
REPO_DEFAULT_TARBALL="https://github.com/${OWNER}/refactor-chain/archive/refs/heads/main.tar.gz"
YES=0; DRYRUN=0; DETAILS=0; UNINSTALL=0; UPDATE=0; ONLY=""; SKIP=""
while [ $# -gt 0 ]; do
  case "$1" in
    --yes|-y) YES=1 ;;
    --dry-run) DRYRUN=1 ;;
...
if [ -z "$SRC" ]; then
  ...
  TB="$TMP/rc.tgz"
  # RCX_TARBALL_URL lets CI point the download path at a locally-built tarball
  # (file://…) so the tarball-layout source resolution is tested without the network.
  URL="${RCX_TARBALL_URL:-$(printf '%s' "$REPO_DEFAULT_TARBALL" | sed "s/ahmedbenaw/${OWNER}/g")}"
  ...
  # The GitHub tarball nests as <repo>-<ref>/.claude-plugin/plugin.json (depth 3 under $TMP),
  # so -maxdepth must reach it. Do NOT pipe to `head` — this script defines a head() function
  # (line ~37) that shadows the command; `sed -n '1...p'` takes the first match instead.
  SRC=$(find "$TMP" -maxdepth 4 -name plugin.json -path '*/.claude-plugin/*' 2>/dev/null | sed -n '1s#/.claude-plugin/plugin.json##p')
  [ -n "$SRC" ] || { bad "could not find plugin in tarball"; exit 1; }
  ok "downloaded"
fi
[ $DETAILS -eq 1 ] && say "${D}source: $SRC${N}"

# version of the source we're about to install (for the install stamp + --update reporting).
# NOTE: do not pipe to `head` — this script defines a head() function (line ~37) that would
# shadow the command. awk reads the first "version" line's value directly.
VERSION=$(awk -F'"' '/"version"[[:space:]]*:/{print $4; exit}' "$SRC/.claude-plugin/plugin.json" 2>/dev/null || true)
...
# ---- plan ----
if [ $UNINSTALL -eq 1 ]; then ACT="uninstall"; elif [ $UPDATE -eq 1 ]; then ACT="update"; else ACT="install"; fi
...
# PLAN_CLAUDE/PLAN_CODEX are numeric 0/1, so the old `-z "$PLAN_ED$PLAN_CLAUDE$PLAN_CODEX"`
# was never empty (min "00") — the "nothing detected" notice never fired. Test each surface.
if [ "$PLAN_CLAUDE" -eq 0 ] && [ "$PLAN_CODEX" -eq 0 ] && [ -z "$PLAN_ED" ]; then skip "no supported surfaces detected"; fi
...
install_claude() {
  ...
  # verify the harness scripts actually landed BEFORE printing success / registering hooks —
  # a partial copy (ENOSPC, perms, bad source) must fail loudly, not leave 6 dangling hooks.
  if [ ! -f "$D/skills/refactor-chain/scripts/orchestrate.mjs" ] || [ ! -f "$D/skills/refactor-chain/scripts/lib/board.mjs" ]; then
    bad "Claude install incomplete — harness scripts did not copy (source: $SRC). Skipping hook registration to avoid dangling hooks."; return 1
  fi
  # version stamp — read by --update to report the installed version
  [ -n "${VERSION:-}" ] && printf '%s\n' "$VERSION" > "$D/skills/refactor-chain/.rcx-version" 2>/dev/null || true
  # commands
...
uninstall_claude() {
  ...
  if NB=$(node_bin); then
    "$NB" - "$D/settings.json" <<'NODEEOF' 2>/dev/null || true
...
NODEEOF
    ...
    # Removing hook entries means editing settings.json safely, which needs Node. Don't strand
    # 6 hooks pointing at now-deleted scripts silently — warn loudly with the manual fix.
    bad "Node not found — removed skills & commands, but LEFT the 6 refactor-chain hook entries in $D/settings.json. Install Node and re-run \`--uninstall\`, or delete the lines whose command contains 'refactor-chain/scripts/' by hand."
  fi
}

# ---- install: codex-home ----
install_codex() {
  CDST="$CODEX_HOME/plugins/refactor-chain"
  mkdir -p "$CDST"
  cp -R "$SRC/." "$CDST/" 2>/dev/null || true
  # NEVER ship a maintainer's local secrets or VCS into the plugin dir: when install runs from
  # a working clone, $SRC contains a token-bearing .env (gitignored) and .git — prune them.
  rm -rf "$CDST/.git" "$CDST/node_modules" 2>/dev/null || true
  rm -f "$CDST"/.env "$CDST"/.env.* 2>/dev/null || true
  SD="$CDST/scripts"
  # verify scripts landed before claiming success (same anti-dangling-hook check as Claude).
  if [ ! -f "$SD/orchestrate.mjs" ] || [ ! -f "$SD/lib/board.mjs" ]; then bad "Codex install incomplete — scripts did not copy; skipping hooks"; return 1; fi
...
if [ $UPDATE -eq 1 ]; then head "Updating…"; else head "Installing…"; fi
...
if [ $UPDATE -eq 1 ]; then
  # --update = check the installed version, then reinstall CLEANLY (clear old refactor-*
  # skills/commands first so a rename between versions can't leave an orphan behind).
  if [ $PLAN_CLAUDE -eq 1 ]; then
    CUR=$(cat "$CLAUDE_HOME/skills/refactor-chain/.rcx-version" 2>/dev/null || true)
    if [ -n "$CUR" ]; then ok "current: v$CUR  →  v${VERSION:-?}"; else skip "no version stamp (fresh or pre-4.6.3 install) — installing v${VERSION:-?}"; fi
    rm -rf "$CLAUDE_HOME/skills/refactor-"* 2>/dev/null || true
    rm -f "$CLAUDE_HOME/commands/refactor"*.md "$CLAUDE_HOME/commands/fix.md" "$CLAUDE_HOME/commands/check.md" 2>/dev/null || true
  fi
  [ $PLAN_CODEX -eq 1 ] && rm -rf "$CODEX_HOME/plugins/refactor-chain" 2>/dev/null || true
fi
...
# ---- verify + summary ----
if [ $UPDATE -eq 1 ]; then head "Updated to v${VERSION:-?}."; else head "Done."; fi
...
say "${D}Tips: /fix (something broke) · /check (review before shipping) · re-run with --update to update, --uninstall to remove.${N}"
