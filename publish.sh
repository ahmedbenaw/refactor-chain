...
#
# It will ask you to paste your GitHub token (input hidden), then cache it in a
# local, GITIGNORED .env so future runs don't prompt. The token is never stored in
# git config, never committed (publish.sh refuses to cache unless git confirms .env is
# ignored), and never echoed. (Power users: GH_TOKEN=... sh publish.sh skips both.)
set -eu
OWNER="ahmedbenaw"
REPO="refactor-chain"
...
# Reuse a saved token from a local .env (gitignored) so you don't re-paste every run.
# File format: one line `GH_TOKEN=ghp_...`. It is only WRITTEN below when git confirms
# .env is ignored, so the --rewrite `git add -A` can never stage/commit it.
ENV_FILE=".env"
TOKEN_FROM_ENV=0
if [ -z "${GH_TOKEN:-}" ] && [ -f "$ENV_FILE" ]; then
  GH_TOKEN=$(awk '/^GH_TOKEN=/{print substr($0,10); exit}' "$ENV_FILE")
  if [ -n "${GH_TOKEN:-}" ]; then TOKEN_FROM_ENV=1; echo "✓ using GitHub token from .env (local, gitignored)"; fi
fi
...
# Cache a freshly-pasted, validated token for next time — ONLY if git confirms .env is
# ignored (otherwise the --rewrite `git add -A` could stage and force-push it → leak).
if [ "$TOKEN_FROM_ENV" = "0" ]; then
  if git check-ignore -q "$ENV_FILE" 2>/dev/null; then
    ( umask 077; printf 'GH_TOKEN=%s\n' "$GH_TOKEN" > "$ENV_FILE" )
    chmod 600 "$ENV_FILE" 2>/dev/null || true
    echo "  ✓ token cached in .env (gitignored, local only) — future publishes won't prompt"
  else
    echo "  ⚠ .env is NOT gitignored — refusing to cache the token there (would risk committing it). Add '.env' to .gitignore first."
  fi
fi
...
code=$(curl -s -o /tmp/rcx-pub.json -w '%{http_code}' -X POST "$API/user/repos" \
  ...
  -d "{\"name\":\"${REPO}\",\"description\":\"Self-diagnosing, self-healing refactor/modernize/fix pipeline for Claude Code, Cowork, Codex, and your editors. Plain-language, behavior-preserving, 57 skills, 40+ languages.\",\"private\":false}")
case "$code" in
  201) echo "  ✓ created" ;;
...
echo "→ 4/5 Creating (or refreshing) the GitHub release with the notes…"
...
case "$code" in
  ...
  422)
    # Release already exists: PATCH its notes so the page never goes stale on a
    # re-publish of the same tag (same id-extraction pattern as the 2b cleanup).
    rel_id=$(curl -s -H "$AUTH" "$API/repos/${OWNER}/${REPO}/releases/tags/${TAG}" | \
             sed -n 's/^[[:space:]]*"id":[[:space:]]*\([0-9]*\),.*/\1/p' | head -1)
    if [ -n "$rel_id" ]; then
      upd=$(curl -s -o /tmp/rcx-rel.json -w '%{http_code}' -X PATCH "$API/repos/${OWNER}/${REPO}/releases/${rel_id}" \
        -H "$AUTH" -H "Accept: application/vnd.github+json" \
        -d "{\"tag_name\":\"${TAG}\",\"name\":\"refactor-chain ${TAG}\",\"body\":${body}}")
      if [ "$upd" = "200" ]; then
        echo "  ✓ release already existed — notes refreshed"
      else
        echo "  ⚠ release exists but refresh returned HTTP $upd"; sed -n '1,3p' /tmp/rcx-rel.json
      fi
    else
      echo "  ✓ release already exists (could not read its id to refresh notes)"
    fi ;;
...
if [ -f "$ENV_FILE" ]; then
  echo "   🔒 Your token is cached in .env (local, gitignored) — future runs won't prompt."
  echo "      Keep .env private; rotate the token if this machine is ever exposed: https://github.com/settings/tokens"
else
