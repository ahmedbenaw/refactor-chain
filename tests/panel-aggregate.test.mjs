...
// ---- S2: string-typed LLM JSON normalizes identically to native types ----
{
  const d = decide(dedupeFindings([{ where: "x:1", cause: "c", severity: "blocker", confidence: "0.9", fix: "do x" }]));
  t("string confidence '0.9' → fix-these-first (not no-go)", d.decision === "fix-these-first", d);
  const d2 = decide(dedupeFindings([{ where: "y:1", cause: "c", severity: "blocker", confidence: "0.4", fix: "do x" }]));
  t("string confidence '0.4' → no-go (low-confidence blocker)", d2.decision === "no-go", d2);
  const bc = dedupeFindings([{ where: "z:1", cause: "c", severity: "worth-fixing", confidence: 0.8, behaviorChanged: "false" }]);
  t("string behaviorChanged 'false' → false", bc[0].behaviorChanged === false, bc[0]);
  const bcT = dedupeFindings([{ where: "z:2", cause: "c", severity: "worth-fixing", confidence: 0.8, behaviorChanged: "true" }]);
  t("string behaviorChanged 'true' → true", bcT[0].behaviorChanged === true, bcT[0]);
  t("non-numeric confidence → 0.5 default", dedupeFindings([{ where: "q:1", cause: "c", severity: "optional", confidence: "high" }])[0].confidence === 0.5);
}

// ---- S2: dedupe merge reconciles verdict (CONFIRMED not masked by a first-inserted SUSPECTED) ----
{
  const merged = dedupeFindings([
    { where: "m.js:9", cause: "same underlying cause", severity: "blocker", confidence: 0.8, verdict: "SUSPECTED" },
    { where: "m.js:9", cause: "same underlying cause", severity: "blocker", confidence: 0.9, verdict: "CONFIRMED" },
  ]);
  t("merge keeps the stronger CONFIRMED verdict", merged.length === 1 && merged[0].verdict === "CONFIRMED", merged);
  const merged2 = dedupeFindings([{ where: "m.js:10", cause: "c", severity: "blocker", confidence: 0.8, verdict: " confirmed " }]);
  t("verdict is trimmed + upper-cased", merged2[0].verdict === "CONFIRMED", merged2[0]);
}
