...
// ---- 4. ship-gate: blocks mid-lane, approves docs / blocked-health ----
{
  const d = fixture(); activeState(d, { phase: "lane", health: "ok" });
  let r = hook("ship-gate.mjs", d, {});
  t("ship-gate blocks when phase=lane", r.json?.decision === "block" && /review gate/.test(r.json?.reason || ""), r.json);
  activeState(d, { phase: "docs", health: "ok" });
  r = hook("ship-gate.mjs", d, {});
  t("ship-gate approves when phase=docs", r.json?.decision === "approve", r.json);
