...
// ---- S5b registry fixes ----
{
  // P4: pascal/delphi merged into ONE systems-family entry (was two dupes wrongly in dotnet)
  const hits = detectLanguages(new Set(), {}, new Set([".pas"]));
  const pas = hits.filter((h) => h.id === "pascal");
  t("P4: .pas → single pascal entry, family systems", pas.length === 1 && pas[0].family === "systems", pas);
  t("P4: delphi id removed (still covered via variant)", !hits.some((h) => h.id === "delphi"), hits.map((h) => h.id));
  // P12: java resolves gradle from a Kotlin-DSL build file (was null → no test cmd)
  const jr = resolveTestFramework(["java"], new Set(["build.gradle.kts"]), {});
  t("P12: java + build.gradle.kts → gradle (not null)", jr?.framework === "gradle", jr);
}
