...
// ---- review board: plan → record → status → aggregate; persists + drops REFUTED ----
{
  ...
  const agg = orch(["board-aggregate"]);
  t("board-aggregate drops REFUTED, keeps the confirmed blocker", agg.json?.total === 1 && agg.json.decision === "fix-these-first" && agg.json.list?.[0]?.where === "a:1", agg.json);
