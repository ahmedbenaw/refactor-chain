...
in `.refactor-chain/board.json` (resumable across compaction). Resolve the scripts dir the same
way the chain does — prefer the plugin root, fall back to the installed skills dir:
`DIR="${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/refactor-chain}/scripts"` (a marketplace-only
install has no `~/.claude/skills/refactor-chain`, so the hardcoded path would not exist).
