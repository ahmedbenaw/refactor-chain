...
**Verify at the cheapest sufficient level — no ceremony.** The point is a confirmed
fact, not a ritual. If the authoritative artifact is already in front of you (the source
file, the actual config), the check IS reading it — do that and answer; do not re-run a
probe to "prove" what the open file already shows. Reach for a live probe (run it, query
it, curl it) only when the truth is NOT in view, or when behavior could differ from what
the text implies. Over-verification is its own failure mode: it burns trust and time, and
it trains the reader to skim past checks that cried wolf. Match the check's cost to the
action's blast radius — a one-line read for a plan note, a live probe before a delete.
