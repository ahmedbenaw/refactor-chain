...
7. Fill `templates/output.md` into a Self-improvement Retro (patterns + the one improvement + the
   corrections-capture proposals + the updated per-repo confidence picture). See
   `references/method.md` for the pattern-detection rules.
...
- **Outputs:** one concrete improvement proposal per run (skill/ruleset/threshold), human-approved only; validated retro appended via `diagnose.mjs learn` (malformed retros rejected).
