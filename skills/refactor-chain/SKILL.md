...
```
Reset writes a improvement retro to the persistent `<project>/.refactor-chain/history.jsonl`; **`refactor-improve`** reflects and proposes one improvement. The next run's confidence prior reads that history — the chain gets smarter per repo.
...
- **`refactor-scope-fence`** — edits stay inside `state.steps[cursor].scope`; adjacent problems are flagged and parked, never silently fixed.
- **`refactor-adversarial-verify`** — every step's "it worked" claim is attacked before `advance`; the review gate red-teams the whole diff.
...
- **Guidelines gate** — `scripts/lib/guidelines.mjs` extract→audit runs in diagnose; `eval` must reach 100% PASS (or an explicitly recorded exception) before the chain may complete.
...
- **Outputs:** `.refactor-chain/state.json` (schema v3), per-step verify records, the write-up, a improvement retro appended to `history.jsonl`.
