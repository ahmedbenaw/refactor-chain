...
async function recommend(ids) {
  ...
  try {
    ...
    const detected = ids.map((id) => ({ id, family: undefined }));
    return { source: registryPath, recommendation: mod.recommendPrinciples(detected) };
  } catch (err) {
    return {
      source: registryPath,
      error: `registry unavailable (${err.code || err.message}) — present the agnostic baseline manually: SOLID, GRASP, DRY/KISS/YAGNI, separation of concerns, cohesion/coupling, Demeter, composition-over-inheritance, CQS, layering, CUPID, Unix, fail-fast, least astonishment.`,
    };
  }
}
