...
**Deterministic helper (shared with the review board).** The exact normalize →
de-dupe → rank → decide logic above is implemented once in
`~/.claude/skills/refactor-chain/scripts/lib/panel-aggregate.mjs` so the gate and the
board can never drift. Feed it the collected findings and it returns the ranked list,
the optional appendix, and the go / fix-these-first / no-go decision:

```
echo '[{"source":"security","findings":[{"where":"a.js:10","cause":"...","severity":"high","confidence":0.9,"fix":"..."}]}]' \
  | node ~/.claude/skills/refactor-chain/scripts/lib/panel-aggregate.mjs aggregate
```

Mark a finding `"behaviorChanged": true` to give it the privileged top-of-tier sort.
Use the helper for the mechanical synthesis; keep the calm-tone framing (§5) for how
you present its output to the human.
