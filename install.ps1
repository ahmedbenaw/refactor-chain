...
# source version (for the install stamp + --update reporting)
$Version = ''
try { $Version = (Get-Content -Raw -Path (Join-Path $Src '.claude-plugin\plugin.json') | ConvertFrom-Json).version } catch { $Version = '' }
...
# 4. show the plan
# ===================================================================
$Act = if ($Uninstall) { 'uninstall' } elseif ($Update) { 'update' } else { 'install' }
...
function Install-Claude([string]$Dir) {
  ...
  # verify the harness scripts actually landed BEFORE stamping / registering hooks — a partial
  # copy must fail loudly, not leave 6 dangling hooks pointing at missing files.
  if (-not (Test-Path (Join-Path $scriptsDst 'orchestrate.mjs')) -or -not (Test-Path (Join-Path $libDst 'board.mjs'))) {
  ...
  if ($Version) { Set-Content -Path (Join-Path $Dir 'skills\refactor-chain\.rcx-version') -Value $Version }
  # commands
...
function Install-Codex {
  ...
  # -Force copies hidden items too (Git marks .git hidden on Windows). NEVER ship a
  # maintainer's local secrets or VCS: prune .git/.env*/node_modules from the plugin dir.
  foreach ($junk in @('.git', 'node_modules', '.env')) {
    $jp = Join-Path $pluginDir $junk
    if (Test-Path $jp) { Remove-Item -Recurse -Force $jp -ErrorAction SilentlyContinue }
  }
  Get-ChildItem -Path $pluginDir -Filter '.env.*' -Force -ErrorAction SilentlyContinue | ForEach-Object { Remove-Item -Force $_.FullName -ErrorAction SilentlyContinue }
  $scriptsDir = Join-Path $pluginDir 'scripts'
  if (-not (Test-Path (Join-Path $scriptsDir 'orchestrate.mjs')) -or -not (Test-Path (Join-Path $scriptsDir 'lib\board.mjs'))) {
...
try {
  ...
  if ($Update) {
    # --update = check installed version, then reinstall CLEANLY (clear old refactor-*
    # skills/commands first so a rename between versions can't leave an orphan behind).
    if ($PlanClaude) {
      $verFile = Join-Path $ClaudeHome 'skills\refactor-chain\.rcx-version'
      $cur = ''
      if (Test-Path $verFile) { $cur = (Get-Content -Raw -Path $verFile).Trim() }
      if ($cur) { Ok "current: v$cur  ->  v$Version" } else { Skip "no version stamp (fresh or pre-4.6.3 install) - installing v$Version" }
      Get-ChildItem -Path (Join-Path $ClaudeHome 'skills') -Directory -Filter 'refactor-*' -ErrorAction SilentlyContinue | ForEach-Object { Remove-Item -Recurse -Force $_.FullName -ErrorAction SilentlyContinue }
      Get-ChildItem -Path (Join-Path $ClaudeHome 'commands') -File -ErrorAction SilentlyContinue | Where-Object { $_.Name -like 'refactor*' -or $_.Name -eq 'fix.md' -or $_.Name -eq 'check.md' } | ForEach-Object { Remove-Item -Force $_.FullName -ErrorAction SilentlyContinue }
    }
    if ($PlanCodex) { Remove-Item -Recurse -Force (Join-Path $CodexHome 'plugins\refactor-chain') -ErrorAction SilentlyContinue }
  }
  ...
  # ---- verify + summary ----
  if ($Update) { Head "Updated to v$Version." } else { Head "Done." }
  ...
  Detail "Tips: /fix (something broke)  /check (review before shipping)  re-run with -Update to update, -Uninstall to remove."
}
finally {
  Invoke-Cleanup
}
