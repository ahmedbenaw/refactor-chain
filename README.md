...
> **By [Ahmed "Ben" Zayed](https://zat.vc)** — Head of Products & Ventures at ZAT.VC · Venture Partner
...
the right lane of **56 native `refactor-*` skills**. It takes a baseline before
...
> 69 commands with examples, worked use cases, and macOS / Linux / Windows notes.
...
- **java** — 9 ordered governance steps: architecture → module-rename → dao-model →
  service → controller → dependency-guard → api-naming → common-extract →
  code-optimize.
...
- **ui** — visual polish: hierarchy, typography, color, layout, components, tokens,
  a11y, mobile.
- **backend** — 9 layered-governance steps, registry-driven (any stack with `lane: "backend"`).
...
| | |
|---|---|
| 🧩 **Skills** | 56 native `refactor-*` skills across 5 lanes |
| ⌨️ **Commands** | 69 plain-language slash commands |
...
Built by Ahmed "Ben" Zayed — Head of Products & Ventures at ZAT.VC [Venture Partner]
