...
export const PRINCIPLES = [
  ...
  { id: "raii-ownership", name: "RAII / ownership discipline", appliesTo: ["cpp", "rust"],
    pitch: "Whoever creates a resource owns freeing it, enforced by scope — leaks and double-frees stop being possible.",
    risks: "A C++/Rust concept — C, Go, and Ada have no destructor-scoped ownership, so don't force it there." },
  { id: "design-by-contract", name: "Design by Contract", appliesTo: ["ada"],
  ...
  { id: "data-oriented", name: "Data-oriented design", appliesTo: ["c", "cpp", "zig", "rust"],
  ...
  { id: "component-purity", name: "Component purity & unidirectional data flow", appliesTo: ["js", "dart"],
    ...
    risks: "UI/component code only — irrelevant to backend Node/Bun services or wasm modules; skip it there." },
  // ── Apple / mobile ─────────────────────────────────────────────────────────
  { id: "value-oriented-swift", name: "Value types & protocol-oriented design", appliesTo: ["apple", "swift", "objective-c"],
    pitch: "Prefer structs/enums with value semantics and compose behavior through protocols; reach for classes only when identity/reference is genuinely needed — fewer shared-mutable-state bugs.",
    risks: "Value semantics can copy more than expected on hot paths — measure before over-applying to large aggregates." },
  // ── Legacy modernization ───────────────────────────────────────────────────
  { id: "strangler-modernization", name: "Seam-first (strangler-fig) modernization", appliesTo: ["legacy", "cobol", "fortran"],
    pitch: "Wrap the legacy core behind a seam, carve out one behavior at a time behind characterization tests, and route callers across gradually — never a big-bang rewrite." },
  // ── Scientific / numerical ─────────────────────────────────────────────────
  { id: "vectorization-repro", name: "Vectorization & reproducible analysis", appliesTo: ["sci", "r", "matlab", "wolfram"],
    pitch: "Operate on whole arrays instead of element loops, and pin seeds, data versions, and environment so a result reproduces on another machine.",
    risks: "Vectorized code can obscure intent and balloon memory — keep a readable reference path for the non-hot cases." },
