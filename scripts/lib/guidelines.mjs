...
switch (cmd) {
  ...
  case "audit": {
    const gj = join(target, ".refactor-chain", "guidelines.json");
    const observed = existsSync(gj) ? JSON.parse(readFileSync(gj, "utf8")) : extract();
  ...
  case "eval": {
    ...
    // accepted exceptions (recorded via decision checkpoints) live next to the manifest
    let exceptions = [];
    try { const parsed = JSON.parse(readFileSync(join(target, ".refactor-chain", "guideline-exceptions.json"), "utf8")); exceptions = Array.isArray(parsed) ? parsed : []; } catch { /* none / unreadable — no exceptions */ }
    const failing = results.filter((r) => !r.pass && !exceptions.includes(r.id));
    out({ pass: results.filter((r) => r.pass).length, total: results.length,
          excepted: exceptions, failing,
