...
/**
 * The lenses to run. ALL of them, always — every lens (architecture, harness, correctness,
 * security, docs-truth) earns its place on any codebase, so there is no stack-specific trimming
 * to do. (Callers may still pass an explicit `--lenses` subset; this is only the default.)
 */
...
export function verifierPromptsFor(lensId, allLensIds, seed, findings) {
  ...
  // Emit the 0-based index `i` with each prompt so the dispatcher records the verdict as
  // {i, verdict, note} using THIS value — not a re-invented (and possibly 1-based) index that
  // would mis-pair verdicts to findings.
  return (findings || []).map((f, i) => ({ i, finding: f, prompt: verifierPrompt(a, f) }));
}

/**
 * Aggregate the board's per-lens results into one ranked ledger + decision.
 * lensResults: [{ lens, findings:[...], verdicts:[{i, verdict, note}] }], where each verdict's
 * `i` is the 0-based index into that lens's `findings` array (see verifierPromptsFor, which
 * emits `i` so the dispatcher pairs correctly).
