...
export const LANGUAGES = [
  ...
  { id: "java", family: "jvm", variants: ["Java SE", "Jakarta EE", "Android"],
    ...
    test: [{ framework: "maven", cmd: "mvn test", needs: "pom.xml" }, { framework: "gradle", cmd: "gradle test", needs: "build.gradle" }, { framework: "gradle", cmd: "gradle test", needs: "build.gradle.kts" }],
  ...
  // One Pascal/Delphi entry (was two duplicates that both claimed .pas and mis-mapped to the
  // dotnet family → a Pascal repo got twelve-factor + ddd-tactical). "delphi" stays reachable
  // via the "Delphi" variant. family:"systems" → agnostic principles, not .NET's.
  { id: "pascal", family: "systems", variants: ["Delphi", "Object Pascal", "Free Pascal", "Pascal Script", "Turbo Pascal", "Lazarus"],
  ...
  { id: "shell", family: "scripting", variants: ["bash", "zsh", "POSIX sh", "fish"], frameworks: ["bats-core", "shellcheck"],
    detect: { manifests: [], exts: [".sh", ".bash", ".zsh"], markers: [] }, // (shebang markers removed — detection never scans file CONTENT, only names/deps)
  ...
  { id: "curl", family: "scripting", variants: ["curl scripts", "HTTP request collections"], frameworks: ["hurl", "httpie collections"],
    detect: { manifests: [], exts: [".curl", ".hurl"], markers: [] }, // ("curl -" marker removed — detection matches names/deps, not file content)
  ...
  { id: "prolog", family: "logic", variants: ["SWI-Prolog", "Visual Prolog", "GNU Prolog"], frameworks: ["plunit"],
    ...
    notes: "prolog registers .pro/.prolog/.plt (NOT .pl — that stays with Perl to avoid a collision; detection matches names/deps, it does not read file content)" },
  // ── Legacy / enterprise ─────────────────────────────────────────────────
