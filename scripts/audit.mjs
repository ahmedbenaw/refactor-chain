...
const TEXT_EXT = /\.(md|mjs|js|ts|json|yml|yaml|sh|ps1|txt|toml|html|css|go)$|^[^.]+$/; // incl .go — the Go installer must be secret/PII-scanned too
...
const isLocalOnly = (p) => p.endsWith(".local.md"); // gitignored working notes — never shipped, not audited
...
for (const f of files) {
  ...
  if (!isText(f) || isLocalOnly(f)) continue;
...
// ---- stale lane label: skills must use the canonical lane set, never "java" ----
{
  const laneRe = /lane(?:\s*[:=]+\s*|\s+∈\s*\{?)\s*["'`]?(java[\w -]*)/i;
  for (const f of files) {
    const rel = relative(ROOT, f);
    if (!rel.startsWith("skills/")) continue;
    if (!isText(f) || isLocalOnly(f)) continue;
    const m = readSafe(f).match(laneRe);
    if (m) finding("stale-lane", f, `lane label "${m[1]}" — java lane was renamed to backend`);
  }
}
...
{
  let major = null;
  try { major = "v" + (JSON.parse(readSafe(join(ROOT, ".claude-plugin", "plugin.json"))).version || "0").split(".")[0]; }
  catch { finding("plugin-manifest", join(ROOT, ".claude-plugin", "plugin.json"), "plugin.json missing or invalid JSON — cannot read version"); }
  const re = new RegExp("refactor-chain@(v\\d+)");
  for (const f of files) {
    if (!major) break; // manifest unreadable — already flagged; skip the version cross-check rather than crash
    const rel = relative(ROOT, f);
...
// ---- frontmatter sanity: description must be YAML-safe (quoted if it contains ": ") ----
{
  for (const f of files) {
    const rel = relative(ROOT, f);
    if (!rel.match(/^skills\/[^/]+\/SKILL\.md$/) && !rel.match(/^commands\/[^/]+\.md$/)) continue;
...
// ---- stub detection: a required resource file that is suspiciously tiny is a stub ----
{
  const RESOURCE_MIN = 120; // bytes — a real method/example/template is never this small
  for (const f of files) {
    const rel = relative(ROOT, f);
    if (!rel.match(/^skills\/[^/]+\/(references|examples|templates)\/.+\.md$/)) continue;
    try { if (statSync(f).size < RESOURCE_MIN) finding("stub-resource", f, `${statSync(f).size} bytes — resource looks stubbed/truncated (min ${RESOURCE_MIN})`); } catch { /* skip */ }
  }
}
...
// ---- installer clean-clear parity ----
// On --update every installer must clear the OLD refactor-* skills and the fix/check commands
// before reinstalling (so a renamed skill can't orphan). Nothing enforced this — the hook-parity
// check above covers hooks only. Assert each installer still references those clear markers, so a
// future change that drops the clean-clear from one installer trips the audit (the "two installers
// implement the same clean-clear differently" divergence class).
{
  const installers = ["install.sh", "install.ps1", "bin/rcx.mjs", "installer/go/main.go"];
  for (const inst of installers) {
    const fp = join(ROOT, inst);
    if (!existsSync(fp)) continue; // hook-parity above already reports a missing installer
    const body = readFileSync(fp, "utf8");
    // must clear old refactor-* skills AND the standalone fix/check commands on update/uninstall.
    if (!/refactor-/.test(body)) finding("clean-clear-parity", fp, "no refactor-* skill clear reference");
    if (!body.includes("fix.md") || !body.includes("check.md")) finding("clean-clear-parity", fp, "does not clear the fix.md/check.md commands (clean-clear divergence)");
  }
}
