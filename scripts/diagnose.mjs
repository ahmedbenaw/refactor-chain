...
function classify(utterance, mode) {
  ...
  // A lone fallback vote with no competing signal (e.g. an empty or non-code directory)
  // must NOT read as certainty — otherwise confidence is 1.0 on nothing. Dampen it so the
  // inScope/careful-clarify logic treats a thin-evidence repo as genuinely uncertain.
  const thinEvidence = topScore <= 1 && secondScore === 0;
  if (thinEvidence) confidence = Math.min(confidence, 0.3);
...
const MATRIX = {
  platform: {
    ...
    backend: "pom.xml/build.gradle(Java) / go.mod / requirements.txt / *.csproj → backend lane if a registry backend-lane language else code",
