...
switch (cmd) {
  ...
  case "advance": {
    const s = readState();
    if (!s) { out({ error: "no active chain" }); process.exit(2); }
    if (s.phase === "baseline") { out({ ok: false, blocked: "record a baseline first (safety-net)", phase: s.phase }); break; }
    const delta = opt("delta");
    if (delta === "drift") { out({ ok: false, blocked: "behavior drifted — roll back this step, do not advance", hint: "git stash apply <checkpoint>" }); break; }
    // adversarial-verify ENFORCED (discipline pack): the step's "it worked" claim must survive the attack pass before advancing.
    if (!flag("adversarial")) { out({ ok: false, blocked: "adversarial-verify: attack this step's claim (refactor-adversarial-verify — wrong assumption / edge case / does it actually run) BEFORE advancing, then re-run `advance --delta legal --adversarial`.", phase: s.phase }); break; }
  ...
  case "fail": {
    const s = readState();
    if (!s) { out({ error: "no active chain" }); process.exit(2); }
    const cur = s.steps[s.cursor]; const reason = opt("reason", "unspecified");
  ...
  case "heal": {
    ...
    const cur = s.steps[s.cursor]; cur.status = "active"; s.health = "ok"; writeState(s);
