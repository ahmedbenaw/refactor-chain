...
try {
  ...
  // scope-fence signal (advisory, never blocks): the current step may declare a
  // scope (state.steps[cursor].scope = [path prefixes]); edits outside it are
  // flagged so adjacent problems get PARKED, not silently fixed.
  const scope = cur?.scope;
  if (Array.isArray(scope) && scope.length && editedFile !== "?" &&
      !scope.some((p) => editedFile.includes(p))) {
    ...
    // scope-fence HARD-FLAG: persist the drift into state.notes so the write-up + improve retro always see it.
